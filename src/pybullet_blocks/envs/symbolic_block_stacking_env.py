...
class SymbolicBlockStackingPyBulletBlocksState(PyBulletBlocksState):
    """A state in the BlockStackingPyBulletBlocksEnv."""

    block_states: Collection[LetteredBlockSymbolicState]
    ...
    def to_observation(self) -> spaces.GraphInstance:
        ...
        for block_state in self.block_states:
            ...
            is_on = block_vec[10] != -1
            letter = chr(int(block_vec[8] + 97)).upper()
        ...
        for block_state in self.block_states:
            ...
            if is_on:
                edge_links.append([node_to_index[on_letter], node_to_index[letter]])
            # print(block_vec)
            # print(is_on)
            # print(letter)
            # print(on_letter)
    ...
    def from_observation(
        ...
        block_states: list[LetteredBlockSymbolicState] = []
        for node in obs.nodes:
            if np.isclose(node[0], 0):  # is robot
                ...
                vec = node[: LetteredBlockSymbolicState.get_dimension()]
                block_state = LetteredBlockSymbolicState.from_vec(vec)
...
@dataclass(frozen=True)
class SymbolicBlockStackingSceneDescription(BaseSceneDescription):
...
class SymbolicBlockStackingPyBulletBlocksEnv(
    ...
    def __init__(
        ...
        if scene_description is None:
            scene_description = SymbolicBlockStackingSceneDescription()

        super().__init__(scene_description, render_mode, use_gui, seed=seed)

        # Set up observation space.
        obs_dim = SymbolicBlockStackingPyBulletBlocksState.get_node_dimension()
    ...
    def set_state(self, state: PyBulletBlocksState) -> None:
        assert isinstance(state, SymbolicBlockStackingPyBulletBlocksState)
        self._banish_all_blocks()
    ...
    def get_state(self) -> SymbolicBlockStackingPyBulletBlocksState:
        block_states = []
        block_height = 2 * self.scene_description.block_half_extents[2]
        for block_id_1 in self.active_block_ids:
            ...
            for block_id_2 in self.active_block_ids:
                if block_id_2 != block_id_1:
                    ...
                    x_pos_1 = block_pose_1.position[0]
                    y_pos_1 = block_pose_1.position[1]
                    z_pos_1 = block_pose_1.position[2]

                    x_pos_2 = block_pose_2.position[0]
                    y_pos_2 = block_pose_2.position[1]
        ...
        return SymbolicBlockStackingPyBulletBlocksState(
            block_states,
            robot_state,
        )
    ...
    def reset(
        ...
        *,
        ...
        assert isinstance(scene_description, SymbolicBlockStackingSceneDescription)
