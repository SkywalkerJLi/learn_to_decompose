...
def calculate_plans_parallel(input_list, plan_function, max_workers=None):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        futures = [
            executor.submit(plan_function, graph, subgoal)
...
def gen_next_scene_graph(return_edge_links=True):
    ...
    for demo in range(num_demonstrations):
        ...
        def generate_scene_graph(current_graph, subgoal_index):
            ...
            scene_data = GraphPairDataset.convert_data(
                current_graph, next_subgoal_graph
            )

            # Forward pass
            with torch.no_grad():
                out = mp_model(
                    scene_data.x, scene_data.edge_index, scene_data.edge_attr
                )
                importance_scores = torch.sigmoid(out)
        ...
        graph_pairs = []
        graph_pairs.append(
            (current_graph, {"goal_piles": [subgoal_piles[subgoal_index]]})
        )
        while subgoal_index < len(subgoal_piles) - 1:
            ...
            graph_pairs.append(
                (next_graph, {"goal_piles": [subgoal_piles[subgoal_index]]})
            )
